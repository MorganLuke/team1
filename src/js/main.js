...
//   }
  async function renderSearchResult(searchString) {
    ...
    searchResults = searchResults.concat(await dataSource.searchCategory("tents", searchString));
  
    // Search other categories
    const categories = ["backpacks", "hammocks", "sleeping-bags"];
    for (let category of categories) {
      searchResults = searchResults.concat(await dataSource.searchCategory(category, searchString));
    }
